...
1. **從範例 DLC 資料夾複製一份**，改名成你的 DLC 名稱。
2. **修改 `config.rpy`**（名稱、作者、描述、版本）。
3. **修改角色定義檔 (`characters.rpy`)、劇本檔 (`events/`)**。
4. **在本機啟動遊戲**，指定 DLC 路徑測試。
5. **打包整個資料夾**，丟到你喜歡的平台（itch.io、GitHub、個人網站等）。

> 💡 **官方不提供社群 DLC 審核或上架服務，請自由發佈並自行宣傳。**
...
如果用戶直接使用原始碼安裝，可以直接將資料夾複製到 `game/` 目錄。
